...
FIXED VERSION - MODIFIED TO DECREASE S COMPARTMENT TO ~2000
"""
...
# --------------------------
# Model parameters (tweak) - MODIFIED
# --------------------------
mu = 0.000014        # Birth/Death rate per day (approximate)
beta = 0.8           # INCREASED from 0.5 to 0.8 - Transmission rate
...
class F(object):
    ...
    def f(self, x_vec, u_vec, mu=0.000014, beta=0.8, sigma=0.2, gamma=0.4, N=10_000_000,
...
def solve_mpc(x0, f_func, dt, horizon_steps=14, u_bounds=((0,1),(0,1),(0,1)),
    ...
    if initial_guess is None:
        # MODIFIED: Start with higher u2 (vaccination) guess
        x0_guess = np.tile([0.0, 0.008, 0.0], horizon_steps)
    else:
    ...
    if not res.success:
        # MODIFIED: fallback with vaccination
        u_first = np.array([0.0, 0.008, 0.0])
        u_full = np.tile(u_first, (horizon_steps, 1))
    else:
...
def simulate_seir_rk4_mpc(f_obj, h_obj, tsim_length=365, dt=1.0,
    ...
    # MODIFIED: Lower target for I and E to encourage more aggressive control
    I_set_const = 0.0005 * N  # Reduced from 0.002
    E_set_const = 0.0005 * N  # Reduced from 0.001
...
def main():
    ...
    x0 = np.array([9995000.0, 400.0, 5000.0, 0.0, 0.0], dtype=float)
    ...
    for option_name, desc in measurement_options:
        ...
        try:
            ...
            t_sim, x_sim, u_sim, y_sim = simulate_seir_rk4_mpc(
                f_obj, h_obj, tsim_length=800, dt=1.0,
    ...
    if 'h_all' in results:
        ...
        ax1.plot(t, x['S'], label='S', linewidth=2)
        ax1.plot(t, x['E'], label='E', linewidth=2)
        ax1.plot(t, x['I'], label='I', linewidth=2)
        ax1.plot(t, x['R'], label='R', linewidth=2)
        ax1.set_xlabel('Time (days)')
        ax1.set_ylabel('Population')
        ax1.set_title('SEIR with RK4 + MPC (Modified for S ~2000)')
        ax1.legend()
        ax1.grid()
        
        # Plot controls
        ax2.plot(t, u[:,0], label='u1 (social distancing)', linewidth=2)
        ax2.plot(t, u[:,1], label='u2 (vaccination)', linewidth=2)
        ax2.plot(t, u[:,2], label='u3 (treatment)', linewidth=2)
        ax2.set_xlabel('Time (days)')
        ax2.set_ylabel('Control Values')
        ax2.set_title('MPC Control Actions')
        ax2.legend()
        ax2.grid()
