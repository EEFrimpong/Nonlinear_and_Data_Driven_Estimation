...
############################################################################################
F = 15.0      # recruitment/birth rate (individuals per time unit)
...
class F(object):
    ...
    def f(self, x_vec, u_vec, return_state_names=False):
        ...
        # Similarly for vaccinated: -(1-ε)β(1-u1)VI = -(1-ε)βVI + (1-ε)βu1·VI
        # So the u1 term contributes: +β·SI·u1 in Ṡ and Ė, and +(1-ε)β·VI·u1 in Ė and V̇
        f1_contribution = u1 * np.array([
            self.beta * S * I,                                  # +βSI term in Ṡ
            -self.beta * (S + (1 - self.epsilon) * V) * I,     # -β[S+(1-ε)V]I term in Ė
            0,                                                  # no u1 term in İ
            0,                                                  # no u1 term in Ṙ
            (1 - self.epsilon) * self.beta * V * I             # +(1-ε)βVI term in V̇
        ])
...
class H(object):
    ...
    def h_newinf(self, x_vec, u_vec, return_measurement_names=False):
        ...
        epsilon = 0.7  # vaccine effectiveness
        newinf = beta * (1 - u1) * (S + (1 - epsilon) * V) * I
...
if __name__ == "__main__":
    ...
    f = F(beta=0.0005, mu=0.02, c=0.2, r=0.05, F_rate=15.0, epsilon=0.7, omega=0.01)
