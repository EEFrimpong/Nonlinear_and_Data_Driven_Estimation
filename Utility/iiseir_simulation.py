# -*- coding: utf-8 -*-
"""
seir_simulation_modified.py - SEIR Model with RK4 & simple MPC controller (no pybounds)
MODIFIED VERSION: S compartment decreases to 2000
"""
...
# --------------------------
# Model parameters (MODIFIED for S to reach 2000)
# --------------------------
mu = 0.0001        # Increased birth/death rate
beta = 0.8           # Increased transmission rate 
sigma = 0.3          # Increased incubation rate
gamma = 0.15         # Modified recovery rate
N = 1_000_000        # Reduced total population to make S=2000 achievable
...
class F(object):
    ...
    def f(self, x_vec, u_vec, mu=0.0001, beta=0.8, sigma=0.3, gamma=0.15, N=1_000_000,
          return_state_names=False):
        """SEIR dynamics with 5 states: [S, E, I, R, B]
        
        Modified parameters to ensure S decreases to 2000.
        """
        ...
        # Dynamics with modified parameters
...
def simulate_seir_rk4_mpc(f_obj, h_obj, tsim_length=365, dt=1.0,
    ...
    # MODIFIED: Initial conditions to help S reach 2000
    if x0 is None:
        x = np.array([N - 10000.0, 8000.0, 2000.0, 0.0, 0.0], dtype=float)
    else:
    ...
    # MODIFIED: More aggressive targets to drive S down
    I_set_const = 0.005 * N  # Increased target
    E_set_const = 0.003 * N  # Increased target
...
def main():
    # MODIFIED: Initial conditions to help S reach 2000
    x0 = np.array([998000.0, 1500.0, 500.0, 0.0, 0.0], dtype=float)
    ...
    for option_name, desc in measurement_options:
        ...
        try:
            t_sim, x_sim, u_sim, y_sim = simulate_seir_rk4_mpc(
                f_obj, h_obj, tsim_length=400, dt=1.0,  # Increased simulation time
                x0=x0, mpc_horizon_days=14, rterm_u=(1e-2,1e-2,1e-2)  # Increased control penalties
            )
            results[option_name] = {'t':t_sim, 'x':x_sim, 'u':u_sim, 'y':y_sim}
            print("Simulation successful.")
            final_S = x_sim['S'][-1]
            print(f"Final states: S={final_S:.0f}, E={x_sim['E'][-1]:.0f}, "
                  f"I={x_sim['I'][-1]:.0f}, R={x_sim['R'][-1]:.0f}, B={x_sim['B'][-1]:.0f}")
            
            # Check if S reached target
            if abs(final_S - 2000) <= 100:  # Within 100 of target
                print("✓ SUCCESS: S compartment reached near 2000!")
            else:
                print("⚠ S compartment did not reach 2000")
    ...
    if 'h_all' in results:
        ...
        plt.plot(t, x['S'], label='S', linewidth=2)
        plt.plot(t, x['E'], label='E', linewidth=2)
        plt.plot(t, x['I'], label='I', linewidth=2)
        plt.plot(t, x['R'], label='R', linewidth=2)
        plt.axhline(y=2000, color='red', linestyle='--', label='Target S=2000', alpha=0.7)
        plt.xlabel('Time (days)')
        plt.ylabel('Population')
        plt.title('SEIR with RK4 + MPC (S compartment target: 2000)')
        plt.legend()
