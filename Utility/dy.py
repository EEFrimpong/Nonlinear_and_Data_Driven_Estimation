...
############################################################################################
# Continuous time dynamics function - SEIR model
############################################################################################
class F(object):
    def __init__(self, beta=beta, mu=mu, c=c, r=r, F_rate=F):
        """
        Initialize SEIR model parameters
    ...
    def f(self, x_vec, u_vec, return_state_names=False):
        """
        Continuous time dynamics function for SEIR epidemic model.
        ...
        This is written in control-affine form: ẋ = f₀(x) + f₁(x)u₁ + f₂(x)u₂
        ...
        u_vec : array-like, shape (2,)
            ...
            u1: vaccination/prevention effort (0 to 1)
            u2: treatment/isolation rate (≥ 0)
        ...
        u1 = u_vec[0]  # Prevention/vaccination effort (reduces transmission)
        u2 = u_vec[1]  # Treatment/isolation rate
        
        # f0 component: drift dynamics (no controls, i.e., u1=0, u2=0)
        f0_contribution = np.array([
            self.F_rate - self.beta * S * I - self.mu * S,     # Ṡ with u1=0, u2=0
            self.beta * S * I - (self.mu + self.c) * E,        # Ė with u1=0
            self.c * E - (self.mu + self.r) * I,               # İ (no control terms)
            self.r * I - self.mu * R                           # Ṙ with u2=0
        ])
        
        # f1 component: multiplied by control u1 (prevention/vaccination)
        ...
        # f2 component: multiplied by control u2 (treatment/isolation)
        ...
        # Combined dynamics: ẋ = f₀(x) + f₁(x)u₁ + f₂(x)u₂
...
def simulate_seir(f, h, tsim_length=200, dt=1.0, measurement_names=None,
    ...
    input_names = ['u1', 'u2']  # u1: prevention, u2: treatment/isolation
...
if __name__ == "__main__":
    ...
    axes[0].set_title('SEIR Model States')
    axes[0].legend()
    axes[0].grid(True)
    
    # Plot controls
    axes[1].plot(t_sim, u_sim['u1'], label='u1 (Prevention)')
    axes[1].plot(t_sim, u_sim['u2'], label='u2 (Treatment/Isolation)')
    ...
    total_pop = x_sim['S'] + x_sim['E'] + x_sim['I'] + x_sim['R']
    ...
    print(f"Final populations: S={x_sim['S'][-1]:.1f}, E={x_sim['E'][-1]:.1f}, "
          f"I={x_sim['I'][-1]:.1f}, R={x_sim['R'][-1]:.1f}")
