...
# ==========================================================
# MODEL F  — unchanged MODEL, only 5 states
# ==========================================================
class F(object):
...
# ==========================================================
# MEASUREMENT H — unchanged measurements
# ==========================================================
class H(object):
...
# ==========================================================
# pybounds-STYLE SIMULATOR OBJECT (dummy)
# ==========================================================
class DummySimulator(object):
    def __init__(self):
        ...
        # replicate the pybounds API
        self.mpc.bounds = self.bounds


# ==========================================================
# MAIN SIMULATION (pybounds-style)
# ==========================================================
def simulate_seir(f, h, tsim_length=365, dt=1.0,
    ...
    if setpoint is None:
        ...
        I_target = 0.0001*N
        E_target = 0.00005*N
        I_set = I_target + (I0-I_target)*np.exp(-tsim/100)
        E_set = E_target + (E0-E_target)*np.exp(-tsim/80)
    else:
        I_set = setpoint['I']
        E_set = setpoint['E']
    ...
    simulator.bounds[('lower','_u','u1')] = 0
    simulator.bounds[('upper','_u','u1')] = 0.9
    simulator.bounds[('lower','_u','u2')] = 0
    simulator.bounds[('upper','_u','u2')] = 0.6
    simulator.bounds[('lower','_u','u3')] = 0
    ...
    if measurement_noise_stds is None:
        ...
        noise_std = np.array([measurement_noise_stds.get(m,0) for m in measurement_names])

    # simulate
    for k in range(nT):

        # solve MPC
        u1, Ufull = solve_mpc(x, f, dt, horizon, E_set[k:], I_set[k:])

        # apply u
        x = rk4_step(f, x, u1, dt)
        x = np.clip(x, 0, N)
    ...
    x_dict = {'S':X[:,0],'E':X[:,1],'I':X[:,2],'R':X[:,3],'B':X[:,4]}
...
def main():
    ...
    for key,desc in measurement_options:
        ...
        t, x, u, y, sim = simulate_seir(f, h, tsim_length=100, dt=1.0)

        print("Final:", x['S'][-1], x['E'][-1], x['I'][-1])
